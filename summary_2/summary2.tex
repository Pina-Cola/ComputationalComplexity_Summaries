...
%Lemma: If a deterministic or nondeterministic TM M decides a language in time (or space) f(n) for a proper complexity function f, then one can turn M into a precise TM M (of the same type) that runs in time (or
%space) O(f(n)) and decides the same language.
